...
def parse_random_draw_pull_for_regex(command_text: str, pattern: str) -> Tuple[Optional[RandomDrawPullParse], Optional[str]]:
    normalized_command = normalize_random_draw_pull_text(command_text)
    normalized_pattern = normalize_random_draw_pull_text(pattern)
    if not normalized_pattern:
        return None, None
    try:
        matched = re.search(normalized_pattern, normalized_command)
    except re.error as exc:
        print("[WARN] Invalid DB regex pattern {0!r}: {1}".format(normalized_pattern, exc))
        return None, None
    if matched is None:
        return None, None
    groups = regex_groups(matched)
    suffix = normalized_command[matched.end() :]
    count, error, consumed = parse_random_draw_pull_suffix(suffix)
    if error:
        return None, error
    if consumed and count is not None:
        return RandomDrawPullParse(count, normalized_command, groups), None
    if "連" in normalize_random_draw_pull_text(suffix) or "回" in normalize_random_draw_pull_text(suffix):
        return None, RANDOM_DRAW_PULL_BLOCKED
    return RandomDrawPullParse(1, normalized_command, groups), None
...
def parse_random_draw_pull_for_reaction(reaction: Dict[str, Any], command_text: str) -> Tuple[Optional[RandomDrawPullParse], Optional[str]]:
    ...
    if parsed is not None or error is not None:
        return parsed, error
    match_type = reaction.get("match_type") or "exact"
    if match_type == "regex":
        parsed, error = parse_random_draw_pull_for_regex(command_text, keyword)
        if parsed is not None or error is not None:
            return parsed, error
    groups = match_pattern(keyword, match_type, command_text)
    if groups is None:
        return None, None
