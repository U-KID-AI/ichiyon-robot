...
- `recent`: `/2/tweets/search/recent` を使う。X Recent Search の範囲。
- `full_archive`: `/2/tweets/search/all` を使う。`lookback_days` で現在から何日前まで検索するか指定。
- 優先順位は `mention_reactions.config_json`、`.env`、デフォルトの順。
- Full-Archive の権限がない時は Bot を落とさず「過去検索が使えません」を返す。

stgで過去検索を試す時は、デッキ検索設定の「検索範囲」を「過去検索」にし、検索対象日数を 14 から開始。必要なら 30 まで増やす。

## 検索ログ

Token と QR 文字列はログに出さない。

出るログ:

```text
deck search query: ...
deck search stats: mode=full_archive, endpoint=full_archive, lookback_days=14, http_status=-, X results=..., media=..., downloaded=..., qr=..., candidates=..., skip_no_media=..., skip_non_photo=..., skip_image_fetch=..., skip_no_qr=..., skip_qr_error=...
```
